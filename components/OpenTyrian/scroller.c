...
const struct about_text_type about_text[] = {{0x30, "----- ~OpenTyrian~ -----"},
                                             {0x00, ""},
    ...
    while(!JE_anyButton()) {
        ...
        if(!ale) {
            for(int i = 0; i < MAX_COINS / 2; i++) {
                ...
                blit_sprite2(
                    VGAScreen, coin->x, coin->y, eShapes[4], coin_defs[coin->type].shape_num + coin->cur_frame);
            }
        }
            ...
            if(current_line + i >= 0) {
                ...
                if(text[i + current_line].effect & 0x20) {
                    draw_font_hv(VGAScreen,
                                 line_x + 1,
                                 line_y,
                                 text[i + current_line].text,
                                 normal_font,
                                 centered,
                                 text[i + current_line].effect & 0x0f,
                                 -10);
                    draw_font_hv(VGAScreen,
                                 line_x - 1,
                                 line_y,
        ...
        if(!ale) {
            ...
            for(uint i = 0; i < COUNTOF(beer); i++) {
                ...
                if(beer[i].x + beer[i].vx > vga_width - 24 ||
                   beer[i].x + beer[i].vx < 0)  // check if the beer hit the sides
                {
                    beer[i].vx = -beer[i].vx;
                }
                beer[i].x += beer[i].vx;
