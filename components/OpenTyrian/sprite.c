...
void IRAM_ATTR blit_sprite_hv_unsafe(
    SDL_Surface *surface, int x, int y, unsigned int table, unsigned int index, Uint8 hue, Sint8 value)
{
    if(index >= sprite_table[table].count || !sprite_exists(table, index)) {
        assert(false);
        return;
...
// does not clip on left or right edges of surface
void IRAM_ATTR
blit_sprite_hv(SDL_Surface *surface, int x, int y, unsigned int table, unsigned int index, Uint8 hue, Sint8 value)
{
    if(index >= sprite_table[table].count || !sprite_exists(table, index)) {
        assert(false);
        return;
...
// does not clip on left or right edges of surface
void IRAM_ATTR
blit_sprite_hv_blend(SDL_Surface *surface, int x, int y, unsigned int table, unsigned int index, Uint8 hue, Sint8 value)
{
    if(index >= sprite_table[table].count || !sprite_exists(table, index)) {
        assert(false);
        return;
...
// does not clip on left or right edges of surface
void IRAM_ATTR
blit_sprite2_filter(SDL_Surface *surface, int x, int y, Sprite2_array sprite2s, unsigned int index, Uint8 filter)
{
    // assert(surface->BitsPerPixel == 8);
    Uint8 *pixels = (Uint8 *) surface->pixels + (y * surface->pitch) + x;
    const Uint8 *const pixels_ll = (Uint8 *) surface->pixels,                          // lower limit
