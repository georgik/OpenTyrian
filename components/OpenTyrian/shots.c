    ...
    for(int z = 0; z < MAX_PWEAPON; z++) {
        if(shotAvail[z] != 0) {
            ...
            if(z != MAX_PWEAPON - 1) {
                ...
                if(shot->shotX < 0 || shot->shotX > 140 || shot->shotY < 0 || shot->shotY > 170) {
                    shotAvail[z] = 0;
                ...
                /*				if (shot->shotTrail != 255)
                                {
                                    ...
                                    {
                                        JE_setupExplosion(shot->shotX - shot->shotXM, shot->shotY - shot->shotYM,
                   shot->shotTrail); } else { JE_setupExplosion(shot->shotX, shot->shotY, shot->shotTrail);
                                    }
                                }*/
...
bool player_shot_move_and_draw(int shot_id,
                               bool *out_is_special,
                               int *out_shotx,
                               int *out_shoty,
                               JE_integer *out_shot_damage,
                               JE_byte *out_blast_filter,
                               JE_byte *out_chain,
                               JE_byte *out_playerNum,
                               JE_word *out_special_radiusw,
                               JE_word *out_special_radiush)
{
    PlayerShotDataType *shot = &playerShotData[shot_id];
    ...
    if(shot_id != MAX_PWEAPON - 1) {
        ...
        *out_shoty = shot->shotY;
        ...
        if(shot->shotX < -34 || shot->shotX > 290 || shot->shotY < -15 || shot->shotY > 190) {
            shotAvail[shot_id] = 0;
            return false;
        }
        ...
        if(shot->shotTrail != 255) {
            if(shot->shotTrail == 98)
                JE_setupExplosion(
                    shot->shotX - shot->shotXM, shot->shotY - shot->shotYM, 0, shot->shotTrail, false, false);
            else
            ...
            *out_special_radiush = sprite(OPTION_SHAPES, sprite_frame - 60001)->height / 2;
            ...
            if(sprite_frame > 500) {
                if(background2 && *out_shoty + shadowYDist < 190 && tmp_shotXM < 100)
                    blit_sprite2_darken(
                        VGAScreen, *out_shotx + 1, *out_shoty + shadowYDist, shapesW2, sprite_frame - 500);
...
JE_integer player_shot_create(JE_word portNum,
                              uint bay_i,
                              JE_word PX,
                              JE_word PY,
                              JE_word mouseX,
