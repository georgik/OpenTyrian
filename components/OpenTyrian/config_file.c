...
static ConfigString string_init_len(const char *s, size_t n)
{
    ...
    if(s == NULL) {
        ...
        char *buffer = is_long ? string.long_buf = (char *) malloc((n + 1) * sizeof(char)) : string.short_buf;
        if(buffer == NULL)
            config_oom();
...
ConfigSection *config_add_section_len(
    Config *config, const char *type, size_t type_len, const char *name, size_t name_len)
{
    assert(config != NULL);
    assert(type != NULL);

    ConfigSection *sections =
        (ConfigSection *) realloc(config->sections, (config->sections_count + 1) * sizeof(ConfigSection));
    if(sections == NULL)
        return NULL;
...
ConfigSection *config_find_sections(Config *config, const char *type, ConfigSection **save)
{
    ...
    ConfigSection *section = save != NULL && *save != NULL ? *save : &config->sections[0];
...
ConfigSection *config_find_section(Config *config, const char *type, const char *name)
{
    ...
    for(ConfigSection *section = &config->sections[0]; section < sections_end; ++section) {
        if(strcmp(config_string_to_cstr(&section->type), type) == 0) {
            ...
            if((section_name == NULL || name == NULL) ? section_name == name
                                                      : strcmp(config_string_to_cstr(&section->name), name) == 0)
                return section;
        }
    }
...
static ConfigOption *append_option(
    ConfigSection *section, const char *key, size_t key_len, const char *value, size_t value_len)
{
    ConfigOption *options =
        (ConfigOption *) realloc(section->options, (section->options_count + 1) * sizeof(ConfigSection));
    if(options == NULL)
        return NULL;
...
ConfigOption *config_set_option_len(
    ConfigSection *section, const char *key, size_t key_len, const char *value, size_t value_len)
{
    assert(section != NULL);
    assert(key != NULL);
...
ConfigOption *config_get_or_set_option_len(
    ConfigSection *section, const char *key, size_t key_len, const char *value, size_t value_len)
{
    assert(section != NULL);
    assert(key != NULL);
...
void config_set_string_option_len(
    ConfigSection *section, const char *key, size_t key_len, const char *value, size_t value_len)
{
...
ConfigOption *config_add_value_len(ConfigOption *option, const char *value, size_t value_len)
{
    ...
    if(option->values_count == 0 && config_string_to_cstr(&option->v.value) != NULL) {
        ...
        ConfigString *values =
            (ConfigString *) realloc(option->v.values, (option->values_count + 1) * sizeof(ConfigString));
        if(values == NULL)
            return NULL;
        ...
        switch(directive) {
            ...
            case SECTION_DIRECTIVE: {
                ...
                section = config_add_section_len(config,
                                                 &buffer[type_start],
                                                 type_length,
                                                 has_name ? &buffer[name_start] : NULL,
                                                 has_name ? name_length : 0);
                if(section == NULL)
                    config_oom();
                option = NULL;
            } break;
            ...
            case LIST_DIRECTIVE: {
                ...
                if(directive == ITEM_DIRECTIVE) {
                    option = config_set_option_len(
                        section, &buffer[key_start], key_length, &buffer[value_start], value_length);
                } else {
                    if(option == NULL || !string_equal_len(&option->key, &buffer[key_start], key_length))
                        option = config_get_or_set_option_len(section, &buffer[key_start], key_length, NULL, 0);
                    if(option != NULL)
                        option = config_add_value_len(option, &buffer[value_start], value_length);
                }
                if(option == NULL)
                    config_oom();
            } break;
