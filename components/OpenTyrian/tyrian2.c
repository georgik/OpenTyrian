...
inline static void blit_enemy(
    SDL_Surface *surface, unsigned int i, signed int x_offset, signed int y_offset, signed int sprite_offset);
    ...
    SDL_Surface *surface, unsigned int i, signed int x_offset, signed int y_offset, signed int sprite_offset)
{
    ...
    const int x = enemy[i].ex + x_offset + tempMapXOfs, y = enemy[i].ey + y_offset;
                    ...
                    if(--enemy[i].eshotwait[j - 1] == 0 && temp3) {
                        ...
                        switch(temp3) {
                            case 252: /* Savara Boss DualMissile */
                                if(enemy[i].ey > 20) {
                                    JE_setupExplosion(
                                        tempX - 8 + tempMapXOfs, tempY - 20 - backMove * 8, -2, 6, false, false);
                                    JE_setupExplosion(
                                        tempX + 4 + tempMapXOfs, tempY - 20 - backMove * 8, -2, 6, false, false);
                                }
                                break;
                            case 251:; /* Suck-O-Magnet */
                                {
                                    const int attractivity =
                                        4 - (abs(player[0].x - tempX) + abs(player[0].y - tempY)) / 100;
                            ...
                            case 255: /* Magneto RePulse!! */
                            {
                                if(difficultyLevel != 1) /*DIF*/
                                {
                                    if(j == 3) {
                                        ...
                                        const int repulsivity =
                                            4 - (abs(player[0].x - tempX) + abs(player[0].y - tempY)) / 20;
                                        if(repulsivity > 0)
...
level_loop:
    ...
    if(!endLevel)  // draw HUD
    {
        ...
        for(uint i = 0; i < 2; ++i) {
            ...
            if(old_weapon_bar[i] != item_power) {
                ...
                int x = twoPlayerMode ? 286 : 289, y = (i == 0) ? (twoPlayerMode ? 6 : 17) : (twoPlayerMode ? 100 : 38);
    ...
    for(int z = 0; z < MAX_PWEAPON; z++) {
        if(shotAvail[z] != 0) {
            ...
            if(!player_shot_move_and_draw(
                   z, &is_special, &tempShotX, &tempShotY, &damage, &temp2, &chain, &playerNum, &tempX2, &tempY2)) {
            ...
            for(b = 0; b < 100; b++) {
                if(enemyAvail[b] == 0) {
                    ...
                    if(z == MAX_PWEAPON - 1) {
                        ...
                        collided =
                            ((enemy[b].enemycycle == 0) && (abs(enemy[b].ex + enemy[b].mapoffset - tempShotX) < 25) &&
                             (abs(enemy[b].ey - tempShotY - 12) < 29)) ||
                            ((enemy[b].enemycycle > 0) && (abs(enemy[b].ex + enemy[b].mapoffset - tempShotX) < 13) &&
                             (abs(enemy[b].ey - tempShotY - 6) < 15));
                    }
                    ...
                    if(collided) {
                        if(chain > 0) {
                            ...
                            b = player_shot_create(
                                0, SHOT_MISC, tempShotX, tempShotY, mouseX, mouseY, chain, playerNum);
                            shotAvail[z] = 0;
                            goto draw_player_shot_loop_end;
                        }
                        ...
                        if(enemy[b].armorleft < 255) {
                            ...
                            for(unsigned int e = 0; e < COUNTOF(enemy); e++) {
                                if(enemy[e].linknum == temp && enemyAvail[e] != 1 && enemy[e].enemyground != 0) {
                                    if(doIced)
                                        enemy[e].iced = doIced;
                                        ...
                                        if((temp3 == b) ||
                                           ((temp != 255) &&
                                            ...
                                            if(enemyDat[enemy[temp3].enemytype].esize != 1)
                                                ...
                                                JE_setupExplosionLarge(
                                                    enemy[temp3].enemyground, enemy[temp3].explonum / 2, tempX, tempY);
                                        }
                                    }
                                }
                            }
    ...
    if(!endLevel) { /*MAIN DRAWING IS STOPPED STARTING HERE*/
        ...
        for(int z = 0; z < ENEMY_SHOT_MAX; z++) {
            if(enemyShotAvail[z] == 0) {
                ...
                if(enemyShot[z].duration-- == 0 || enemyShot[z].sy > 190 || enemyShot[z].sy <= -14 ||
                   enemyShot[z].sx > 275 || enemyShot[z].sx <= 0) {
                    enemyShotAvail[z] = true;
                } else  // check if shot collided with player
                {
                    for(uint i = 0; i < (twoPlayerMode ? 2 : 1); ++i) {
                        if(player[i].is_alive && enemyShot[z].sx > player[i].x - (signed) player[i].shot_hit_area_x &&
    ...
    if(isNetworkGame) {
        if(!reallyEndLevel) {
            Uint16 requests = (pauseRequest == true) | (inGameMenuRequest == true) << 1 |
                              (skipLevelRequest == true) << 2 | (nortShipRequest == true) << 3;
            ...
            if(network_state_update()) {
                assert(SDLNet_Read16(&packet_state_in[0]->data[26]) ==
                       SDLNet_Read16(&packet_state_out[network_delay]->data[26]));

                requests = SDLNet_Read16(&packet_state_in[0]->data[14]) ^
                           SDLNet_Read16(&packet_state_out[network_delay]->data[14]);
                if(requests & 1) {
                    JE_pauseGame();
                }
                if(requests & 2) {
                ...
                for(int i = 0; i < 2; i++) {
                    if(SDLNet_Read16(&packet_state_in[0]->data[18 + i * 2]) !=
                           SDLNet_Read16(&packet_state_out[network_delay]->data[18 + i * 2]) ||
                       SDLNet_Read16(&packet_state_in[0]->data[20 + i * 2]) !=
                           SDLNet_Read16(&packet_state_out[network_delay]->data[20 + i * 2])) {
                        char temp[64];
                ...
                if(s[0] == ']') {
                    switch(s[1]) {
                        ...
                        case 'Q':
                            ...
                            if(twoPlayerMode) {
                                for(uint i = 0; i < 2; ++i)
                                    snprintf(levelWarningText[i],
                                             sizeof(*levelWarningText),
                                             "%s %lu",
                                             miscText[40],
                                             player[i].cash);
                            ...
                            if(jumpBackToEpisode1 && !twoPlayerMode) {
                                ...
                                if(superArcadeMode < SA_ENGAGE) {
                                    if(SANextShip[superArcadeMode] == SA_ENGAGE) {
                                        ...
                                        JE_dString(
                                            VGAScreen, JE_fontCenter(buffer, FONT_SHAPES), 100, buffer, FONT_SHAPES);

                                        sprintf(buffer, "Or play... %s", specialName[7]);
                                        JE_dString(VGAScreen, 80, 180, buffer, SMALL_FONT_SHAPES);
                                    } else {
                                        JE_dString(VGAScreen,
                                                   JE_fontCenter(superShips[0], FONT_SHAPES),
                                                   30,
                                                   superShips[0],
                                                   FONT_SHAPES);
                                        JE_dString(
                                            VGAScreen,
                                            JE_fontCenter(superShips[SANextShip[superArcadeMode]], SMALL_FONT_SHAPES),
                                            100,
                                            superShips[SANextShip[superArcadeMode]],
                                            SMALL_FONT_SHAPES);
                                    }

                                    if(SANextShip[superArcadeMode] < SA_NORTSHIPZ)
                                        blit_sprite2x2(VGAScreen,
                                                       148,
                                                       70,
                                                       shapes9,
                                                       ships[SAShip[SANextShip[superArcadeMode] - 1]].shipgraphic);
                                    else if(SANextShip[superArcadeMode] == SA_NORTSHIPZ)
                                        trentWin = true;

                                    sprintf(buffer, "Type %s at Title", specialName[SANextShip[superArcadeMode] - 1]);
                                    JE_dString(VGAScreen,
                                               JE_fontCenter(buffer, SMALL_FONT_SHAPES),
                                               160,
                                               buffer,
                                               SMALL_FONT_SHAPES);
                                    JE_showVGA();
...
bool JE_titleScreen(JE_boolean animate)
{
    ...
    JE_word waitForDemo;
    JE_byte menu = 0;
    JE_boolean redraw = true, fadeIn = false;
...
// heap_caps_check_integrity_all(true);
    ...
    if(isNetworkGame) {
        ...
        memcpy(VGAScreen2->pixels, VGAScreen->pixels, VGAScreen2->pitch * VGAScreen2->h);
        JE_dString(VGAScreen,
                   JE_fontCenter("Waiting for other player.", SMALL_FONT_SHAPES),
                   140,
        ...
        if(thisPlayerNum == 1) {
            ...
            JE_dString(VGAScreen,
                       JE_fontCenter(networkText[4 - 1], SMALL_FONT_SHAPES),
                       140,
                       networkText[4 - 1],
                       SMALL_FONT_SHAPES);
            JE_showVGA();
        ...
        do {
            ...
            if(redraw) {
                ...
                if(animate) {
                    ...
                    draw_font_hv_shadow(
                        VGAScreen, 2, 192, opentyrian_version, small_font, left_aligned, 15, 0, false, 1);
                    printf("memcpy");
                    memcpy(VGAScreen2->pixels, VGAScreen->pixels, VGAScreen2->pitch * VGAScreen2->h);
                ...
                if(arcade_code_i[i] > 0 && arcade_code_i[i] == strlen(specialName[i])) {
                    if(i + 1 == SA_DESTRUCT) {
                        ...
                        if(select_episode() && select_difficulty()) {
                            ...
                            JE_clr256(VGAScreen);
                            JE_dString(
                                VGAScreen, JE_fontCenter(superShips[0], FONT_SHAPES), 30, superShips[0], FONT_SHAPES);
                            JE_dString(VGAScreen,
                                       JE_fontCenter(superShips[i + 1], SMALL_FONT_SHAPES),
                                       100,
                                       superShips[i + 1],
                                       SMALL_FONT_SHAPES);
...
uint JE_makeEnemy(struct JE_SingleEnemyType *enemy, Uint16 eDatI, Sint16 uniqueShapeTableI)
{
    ...
    if(sprite2s != NULL)
        ...
        // maintain buggy Tyrian behavior (use shape table value from previous enemy that occupied this index in the
        // enemy array)
    ...
    switch(eventRec[eventLoc - 1].eventtype) {
        ...
        case 33: /* Enemy From other Enemies */
            if(!((eventRec[eventLoc - 1].eventdat == 512 || eventRec[eventLoc - 1].eventdat == 513) &&
                 (twoPlayerMode || onePlayerAction || superTyrian))) {
        ...
        case 70:
            if(eventRec[eventLoc - 1].eventdat2 == 0) { /*1-10*/
                bool found = false;
                ...
                if(!found)
                    JE_eventJump(eventRec[eventLoc - 1].eventdat);
                      ...
                      (eventRec[eventLoc - 1].eventdat3 == 0 ||
                       !JE_searchFor(eventRec[eventLoc - 1].eventdat3, NULL)) &&
                      (eventRec[eventLoc - 1].eventdat4 == 0 ||
                       !JE_searchFor(eventRec[eventLoc - 1].eventdat4, NULL))) {
                JE_eventJump(eventRec[eventLoc - 1].eventdat);
            }
            break;

        case 71:
            if(((((intptr_t) mapYPos - (intptr_t) &megaData1.mainmap) / sizeof(JE_byte *)) * 2) <=
               (unsigned) eventRec[eventLoc - 1].eventdat2) {
                JE_eventJump(eventRec[eventLoc - 1].eventdat);
            }
            break;
        ...
        case 75: {
            ...
            for(temp = 0; temp < 100; temp++) {
                if(enemyAvail[temp] == 0 && enemy[temp].eyc == 0 &&
            ...
            if(temp_no_clue) {
                JE_byte enemy_i;
                do {
                    temp = (mt_rand() % (eventRec[eventLoc - 1].eventdat2 + 1 - eventRec[eventLoc - 1].eventdat)) +
                ...
                if(eventRec[eventLoc - 1].eventdat4 > 0) { /*Skip*/
...
void JE_whoa(void)
{
    ...
    do {
        ...
        for(i = screenSize - bottomBorder, j = topBorder / 2; i > 0; i--, j++) {
            offset = j + i / 8192 - 4;
            color = (TempScreen2[offset] * 12 + TempScreen1[offset - VGAScreenSeg->pitch] + TempScreen1[offset - 1] +
                     TempScreen1[offset + 1] + TempScreen1[offset + VGAScreenSeg->pitch]) /
                    16;
...
void draw_boss_bar(void)
{
    ...
    unsigned int bars = (boss_bar[0].link_num != 0 ? 1 : 0) + (boss_bar[1].link_num != 0 ? 1 : 0);
    ...
    for(unsigned int b = 0; b < bars; b++) {
        unsigned int x = (bars == 2) ? ((b == 0) ? 125 : 185)
