...
static inline const char *config_string_to_cstr(const ConfigString *string)
{
    ...
    return is_long ? string->long_buf : string->short_buf;
}
...
extern ConfigSection *config_add_section_len(
...
extern ConfigOption *config_set_option_len(
...
extern ConfigOption *config_get_or_set_option_len(
    ConfigSection *section, const char *key, size_t key_len, const char *value, size_t value_len);
...
extern void config_set_string_option_len(
    ConfigSection *section, const char *key, size_t key_len, const char *value, size_t value_len);
...
{
    assert(option != NULL);
    return option->values_count > 0 || config_string_to_cstr(&option->v.value) == NULL;
}
...
{
    assert(option != NULL);
    return option->values_count == 0 ? (config_string_to_cstr(&option->v.value) == NULL ? 0 : 1) : option->values_count;
}
...
#define foreach_option_value(string_value, option)                                                                \
    for(ConfigOption *_option = (option); _option != NULL; _option = NULL)                                        \
        for(ConfigString *_values_begin = _option->values_count == 0 ? &_option->v.value : &_option->v.values[0], \
                         *_values_end = _option->values_count == 0 ? _values_begin + 1                            \
                                                                   : &_option->v.values[_option->values_count],   \
                         *_value = _values_begin;                                                                 \
            _value < _values_end;                                                                                 \
            ++_value)                                                                                             \
            for(const char *(string_value) = config_string_to_cstr(_value); (string_value) != NULL;               \
                (string_value) = NULL)
