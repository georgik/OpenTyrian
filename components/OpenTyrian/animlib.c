...
int JE_findPage(unsigned int framenumber)
{
    ...
    for(i = 0; i < FileHeader.nlps; i++) {
        if(PageHeader[i].baseRecord <= framenumber && PageHeader[i].baseRecord + PageHeader[i].nRecords > framenumber) {
            return (i);
        }
    }
...
int JE_loadAnim(const char *filename)
{
    ...
    if(memcmp(temp, "LPF ", 4) != 0 || FileHeader.nlps == 0 || FileHeader.nRecords == 0 || FileHeader.nlps > 256 ||
       FileHeader.nRecords > 65535) {
        efclose(InFile);
        return (-1);
    }
     ...
     * padding */
    if(fileSize < (FileHeader.nlps - 1) * ANI_PAGE_SIZE + ANIM_OFFSET + PageHeader[FileHeader.nlps - 1].nBytes +
