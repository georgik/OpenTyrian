...
void JE_helpSystem(JE_byte startTopic)
{
    ...
    do {
        ...
        if(page > 0) {
            ...
            JE_dString(
                VGAScreen, JE_fontCenter(topicName[temp2], SMALL_FONT_SHAPES), 1, topicName[temp2], SMALL_FONT_SHAPES);
        }
            ...
            case 0:
                ...
                do {
                    for(temp = 1; temp <= TOPICS; temp++) {
                        ...
                        JE_dString(VGAScreen,
                                   JE_fontCenter(topicName[temp], SMALL_FONT_SHAPES),
                                   temp * 20 + 40,
                                   buf,
                                   SMALL_FONT_SHAPES);
                    }
...
ulong JE_getCost(JE_byte itemType, JE_word itemNum)
{
    ...
    switch(itemType) {
        ...
        case 4: {
            ...
            const uint port = itemType - 3, item_power = player[0].items.weapon[port].power - 1;
...
void JE_loadScreen(void)
{
    ...
    do {
        ...
        JE_dString(VGAScreen,
                   JE_fontCenter(miscText[38 + screen - 1], FONT_SHAPES),
                   5,
                   miscText[38 + screen - 1],
                   FONT_SHAPES);
...
void JE_nextEpisode(void)
{
    ...
    if(episodeNum == initial_episode_num && !gameHasRepeated && episodeNum != EPISODE_AVAILABLE && !isNetworkGame &&
       !constantPlay) {
        JE_highScoreCheck();
    }
    ...
    if(jumpBackToEpisode1) {
        ...
        if(episodeNum == 1 && !isNetworkGame && !constantPlay) {
            // JE_loadOrderingInfo();
        }

        if(episodeNum > 2 && !constantPlay) {
            JE_playCredits();
        }
    ...
    JE_dString(VGAScreen,
               JE_fontCenter(episode_name[episodeNum], SMALL_FONT_SHAPES),
               130,
               episode_name[episodeNum],
               SMALL_FONT_SHAPES);
...
void JE_highScoreScreen(void)
{
    ...
    do {
        if(episodeAvail[x - 1]) {
            ...
            JE_dString(
                VGAScreen, JE_fontCenter(episode_name[x], SMALL_FONT_SHAPES), 30, episode_name[x], SMALL_FONT_SHAPES);

            /* Player 1 */
            temp = (x * 6) - 6;

            JE_dString(
                VGAScreen, JE_fontCenter(miscText[47 - 1], SMALL_FONT_SHAPES), 55, miscText[47 - 1], SMALL_FONT_SHAPES);
            ...
            for(z = 0; z < 3; z++) {
                ...
                JE_textShade(VGAScreen,
                             250,
                             ((z + 1) * 10) + 65,
                             difficultyNameB[difficulty],
                             15,
            ...
            JE_dString(VGAScreen,
                       JE_fontCenter(miscText[48 - 1], SMALL_FONT_SHAPES),
                       120,
                       miscText[48 - 1],
                       SMALL_FONT_SHAPES);
            ...
            for(z = 0; z < 3; z++) {
                ...
                sprintf(scoretemp,
                        "~#%d:~ %d",
                        z + 1,
                        (int) saveFiles[temp + z].highScore1); /* Not .highScore2 for some reason */
                JE_textShade(VGAScreen,
                             250,
                             ((z + 1) * 10) + 125,
                             difficultyNameB[difficulty],
                ...
                JE_textShade(
                    VGAScreen, 110, ((z + 1) * 10) + 125, saveFiles[temp + z].highScoreName, 15, 2, FULL_SHADE);
            }
...
JE_boolean JE_inGameSetup(void)
{
    ...
    do {
        ...
        for(x = 0; x < 6; x++) {
            JE_outTextAdjust(
                VGAScreen, 10, (x + 1) * 20, inGameText[x], 15, ((sel == x + 1) << 1) - 4, SMALL_FONT_SHAPES, true);
        }

        JE_outTextAdjust(
            VGAScreen, 120, 3 * 20, detailLevel[processorType - 1], 15, ((sel == 3) << 1) - 4, SMALL_FONT_SHAPES, true);
        JE_outTextAdjust(
            VGAScreen, 120, 4 * 20, gameSpeedText[gameSpeed - 1], 15, ((sel == 4) << 1) - 4, SMALL_FONT_SHAPES, true);
                ...
                case SDL_SCANCODE_RETURN:
                    ...
                    switch(sel) {
                        ...
                        case 6:
                            ...
                            if(isNetworkGame) { /*Tell other computer to exit*/
                                haltGame = true;
                                playerEndLevel = true;
                            }
                            break;
...
void JE_highScoreCheck(void)
{
    ...
    for(int temp_p = 0; temp_p < (twoPlayerMode ? 2 : 1); ++temp_p) {
        ...
        int slot;
        const int first_slot = (initial_episode_num - 1) * 6 + (twoPlayerMode ? 3 : 0), slot_limit = first_slot + 3;
        ...
        if(slot < slot_limit) {
...

            {
                ...
                do {
                    ...
                    JE_dString(VGAScreen,
                               JE_fontCenter(miscText[temp3 - 1], SMALL_FONT_SHAPES),
                               30,
                               miscText[temp3 - 1],
                               SMALL_FONT_SHAPES);
                    ...
                    if(mouseButton > 0) {
                        ...
                        switch(lastkey_char) {
                            ...
                            // 	break;
                            default:
                                if(temp < 28 && (validkey || (lastkey_char >= 'A' && lastkey_char <= 'Z') ||
                                                 (lastkey_char >= '0' && lastkey_char <= '9'))) {
                ...
                for(int i = first_slot; i < slot_limit; ++i) {
                    if(i != slot) {
                        ...
                        JE_textShade(VGAScreen,
                                     150,
                                     ((i - first_slot + 1) * 12) + 65,
                                     saveFiles[i].highScoreName,
                                     15,
                                     2,
                                     FULL_SHADE);
                    }
...
bool load_next_demo(void)
{
    ...
    demo_file =
        dir_fopen_die(data_dir(), demo_filename, "rb");  // TODO: only play demos from existing file (instead of dying)
    ...
    JE_initEpisode(temp);
    efread(levelName, 1, 10, demo_file);
    levelName[10] = '\0';
...
void JE_playCredits(void)
{
    ...
    for(int ticks = 0; ticks < ticks_max; ++ticks) {
        ...
        blit_sprite_hv(VGAScreenSeg,
                       319 - sprite(EXTRA_SHAPES, currentpic)->width,
                       100 - (sprite(EXTRA_SHAPES, currentpic)->height / 2),
                       EXTRA_SHAPES,
                       currentpic,
                       0x0,
                       fade - 15);
        ...
        for(int line = bottom_line - 10; line < bottom_line; ++line) {
            if(line >= 0 && line < lines_max) {
                if(strcmp(&credstr[line][0], ".") != 0 && strlen(credstr[line])) {
                    ...
                    JE_outTextAdjust(
                        VGAScreen, x + abs((y / 18) % 4 - 2) - 1, y - 1, text, color, -8, SMALL_FONT_SHAPES, false);
            ...
            if(mouseButton > 0) {
                ...
                switch(lastkey_sym) {
                    ...
                    case SDL_SCANCODE_SPACE:
                        ...
                        break;
                    default:
                        if(temp < 14 && (validkey || (lastkey_char >= 'A' && lastkey_char <= 'Z') ||
                                         (lastkey_char >= '0' && lastkey_char <= '9'))) {
                            JE_playSampleNum(S_CURSOR);
                            stemp[temp] = lastkey_char;
                            temp++;
...
void JE_mainKeyboardInput(void)
{
    ...
    /* {!Activate Nort Ship!} */
    if(keysactive[SDL_SCANCODE_F2] && keysactive[SDL_SCANCODE_F4] && keysactive[SDL_SCANCODE_F6] &&
       keysactive[SDL_SCANCODE_F7] && keysactive[SDL_SCANCODE_F9] && keysactive[SDL_SCANCODE_BACKSLASH] &&
    ...
    if(!isNetworkGame && !twoPlayerMode && !superTyrian && superArcadeMode == SA_NONE) {
        ...
        if(keysactive[SDL_SCANCODE_F2] && keysactive[SDL_SCANCODE_F3] &&
           (keysactive[SDL_SCANCODE_F4] || keysactive[SDL_SCANCODE_F5])) {
            for(uint i = 0; i < COUNTOF(player); ++i)
                player[i].armor = 0;
    ...
    /* {CHEAT-SKIP LEVEL} */
    if(keysactive[SDL_SCANCODE_F2] && keysactive[SDL_SCANCODE_F6] &&
       (keysactive[SDL_SCANCODE_F7] || keysactive[SDL_SCANCODE_F8]) && !keysactive[SDL_SCANCODE_F9] && !superTyrian &&
       superArcadeMode == SA_NONE) {
        if(isNetworkGame) {
...
void JE_pauseGame(void)
{
    ...
    do {
        ...
        if((newkey && lastkey_sym != SDL_SCANCODE_LCTRL && lastkey_sym != SDL_SCANCODE_RCTRL &&
            lastkey_sym != SDL_SCANCODE_LALT && lastkey_sym != SDL_SCANCODE_RALT) ||
...
redo:
    ...
    if(!this_player->is_alive) {
        ...
        if(this_player->exploding_ticks == 0) {
            ...
            // as if instant death weren't enough, player also gets infinite lives in order to enjoy an infinite number
            // of deaths -_-
        ...
        *mouseY_ = this_player->y;
                ...
                if((inputDevice == 0 || inputDevice >= 3) && joysticks > 0) {
                    ...
                    for(; j < j_max; j++) {
                        ...
                        if(joystick[j].analog) {
                            ...
                            this_player->x += (joystick[j].direction[3] ? -CURRENT_KEY_SPEED : 0) +
                                              (joystick[j].direction[1] ? CURRENT_KEY_SPEED : 0);
                            this_player->y += (joystick[j].direction[0] ? -CURRENT_KEY_SPEED : 0) +
                                              (joystick[j].direction[2] ? CURRENT_KEY_SPEED : 0);
                        }
        ...
        if(moveOk) {
            ...
            if(twoPlayerMode && !twoPlayerLinked && this_player->x == *mouseX_ && this_player->y == *mouseY_ &&
               abs(player[0].x - player[1].x) < 8 && abs(player[0].y - player[1].y) < 8 && player[0].is_alive &&
               player[1].is_alive && !galagaMode) {
    ...
    if(this_player->is_alive && !endLevel) {
        ...
        } else /*twoPlayerLinked*/
        {
            ...
            b = player_shot_create(0,
                                   SHOT_MISC,
                                   this_player->x + 1 + roundf(sinf(linkGunDirec + 0.2f) * 26),
                                   this_player->y + roundf(cosf(linkGunDirec + 0.2f) * 26),
                                   *mouseX_,
                                   *mouseY_,
                                   148,
                                   playerNum_);
            ...
            b = player_shot_create(0,
                                   SHOT_MISC,
                                   this_player->x + 1 + roundf(sinf(linkGunDirec) * 26),
                                   this_player->y + roundf(cosf(linkGunDirec) * 26),
                                   *mouseX_,
                                   *mouseY_,
                                   147,
                                   playerNum_);
            ...
            if(shotRepeat[SHOT_REAR] > 0) {
                ...
                b = player_shot_create(0,
                                       SHOT_REAR,
                                       this_player->x + 1 + roundf(sinf(linkGunDirec) * 20),
                                       this_player->y + roundf(cosf(linkGunDirec) * 20),
                                       *mouseX_,
                                       *mouseY_,
                                       linkGunWeapons[this_player->items.weapon[REAR_WEAPON].id - 1],
                                       playerNum_);
    ...
    if(!endLevel) {
        ...
        if(shipGr_ == 0) {
            if(background2) {
                blit_sprite2x2_darken(VGAScreen,
                                      this_player->x - 17 - mapX2Ofs + 30,
                                      this_player->y - 7 + shadowYDist,
                                      *shapes9ptr_,
                                      ship_sprite + 13);
                blit_sprite2x2_darken(VGAScreen,
                                      this_player->x + 7 - mapX2Ofs + 30,
                                      this_player->y - 7 + shadowYDist,
                                      *shapes9ptr_,
                                      ship_sprite + 51);
                if(superWild) {
                    blit_sprite2x2_darken(VGAScreen,
                                          this_player->x - 16 - mapX2Ofs + 30,
                                          this_player->y - 7 + shadowYDist,
                                          *shapes9ptr_,
                                          ship_sprite + 13);
                    blit_sprite2x2_darken(VGAScreen,
                                          this_player->x + 6 - mapX2Ofs + 30,
                                          this_player->y - 7 + shadowYDist,
                                          *shapes9ptr_,
                                          ship_sprite + 51);
                }
            ...
            if(background2) {
                blit_sprite2x2_darken(VGAScreen,
                                      ...
                                      *shapes9ptr_,
                                      220);
                blit_sprite2x2_darken(
                    VGAScreen, this_player->x + 7 - mapX2Ofs + 30, this_player->y - 7 + shadowYDist, *shapes9ptr_, 222);
            }
        } else {
            if(background2) {
                blit_sprite2x2_darken(VGAScreen,
                                      this_player->x - 5 - mapX2Ofs + 30,
                                      this_player->y - 7 + shadowYDist,
                                      *shapes9ptr_,
                                      ship_sprite);
                if(superWild) {
                    blit_sprite2x2_darken(VGAScreen,
                                          this_player->x - 4 - mapX2Ofs + 30,
                                          this_player->y - 7 + shadowYDist,
                                          *shapes9ptr_,
                                          ship_sprite);
                }
        ...
        if(this_player->invulnerable_ticks > 0) {
            ...
            if(shipGr_ == 0) {
                blit_sprite2x2_blend(
                    VGAScreen, this_player->x - 17, this_player->y - 7, *shapes9ptr_, ship_sprite + 13);
    ...
    if(moveOk) {
        if(this_player->is_alive) {
            if(!endLevel) {
                    ...
                    for(temp = min - 1; temp < max; temp++) {
                        ...
                        if(item > 0) {
                            if(shotRepeat[temp] > 0) {
                                ...
                                b = player_shot_create(item,
                                                       temp,
                                                       this_player->x,
                                                       this_player->y,
                                                       *mouseX_,
                                                       *mouseY_,
                                                       weaponPort[item].op[item_mode][item_power],
                                                       playerNum_);
                            }
                        }
                    }
                ...
                if(playerNum_ == 2) {
                    if(!twoPlayerLinked)
                        blit_sprite2(VGAScreen,
                                     this_player->x + (shipGr_ == 0) + 1,
                                     this_player->y - 13,
                                     eShapes[5],
                                     77 + chargeLevel + chargeGr * 19);
                    ...
                    if(chargeLevel > 0) {
                        fill_rectangle_xy(
                            VGAScreenSeg, 269, 107 + (chargeLevel - 1) * 3, 275, 108 + (chargeLevel - 1) * 3, 193);
                    }
                    ...
                    if(chargeLevel > 0)
                        fill_rectangle_xy(
                            VGAScreenSeg, 269, 107 + (chargeLevel - 1) * 3, 275, 108 + (chargeLevel - 1) * 3, 204);
                    ...
                    if(shotRepeat[SHOT_P2_CHARGE] > 0) {
                        ...
                        b = player_shot_create(
                            ...
                            *mouseX_,
                            *mouseY_,
                            chargeGunWeapons[player[1].items.weapon[REAR_WEAPON].id - 1] + chargeLevel,
                            playerNum_);
                ...
                if(player[temp - 1].superbombs > 0) {
                    if(shotRepeat[SHOT_P1_SUPERBOMB + temp - 1] > 0) {
                        ...
                        b = player_shot_create(16,
                                               SHOT_P1_SUPERBOMB + temp - 1,
                                               this_player->x,
                                               this_player->y,
                                               *mouseX_,
                                               *mouseY_,
                ...
                else if(this_player->sidekick[LEFT_SIDEKICK].style == 4 ||
                        this_player->sidekick[RIGHT_SIDEKICK].style == 4)
                ...
                switch(this_player->sidekick[LEFT_SIDEKICK].style) {
                    ...
                    case 2:  // front-mounted
                        ...
                        break;
                    case 4:  // orbitting
                        this_player->sidekick[LEFT_SIDEKICK].x =
                            this_player->x + roundf(sinf(optionSatelliteRotate) * 20);
                        this_player->sidekick[LEFT_SIDEKICK].y =
                            this_player->y + roundf(cosf(optionSatelliteRotate) * 20);
                        break;
                }

                switch(this_player->sidekick[RIGHT_SIDEKICK].style) {
                    case 4:  // orbitting
                        this_player->sidekick[RIGHT_SIDEKICK].x =
                            this_player->x - roundf(sinf(optionSatelliteRotate) * 20);
                        this_player->sidekick[RIGHT_SIDEKICK].y =
                            this_player->y - roundf(cosf(optionSatelliteRotate) * 20);
                        break;
                    case 1:  // trailing
                    case 3:
                    ...
                    case 2:  // front-mounted
                        if(!optionAttachmentLinked) {
                            ...
                            if(optionAttachmentMove >= -2) {
                                ...
                                if(abs(this_player->sidekick[RIGHT_SIDEKICK].y - (this_player->y - 20)) +
                                       abs(this_player->sidekick[RIGHT_SIDEKICK].x - this_player->x) <
                                   8) {
                            ...
                            {
                                optionAttachmentMove += 1 + optionAttachmentReturn;
                                JE_setupExplosion(this_player->sidekick[RIGHT_SIDEKICK].x + 1,
                            ...
                            if(shotRepeat[shot_i] > 0) {
                                ...
                                if(ammo_max > 0)  // sidekick has limited ammo
                                {
                                    ...
                                    if(button[1 + i] && this_player->sidekick[i].ammo > 0) {
                                        b = player_shot_create(this_option->wport,
                                                               shot_i,
                                                               this_player->sidekick[i].x,
                                                               this_player->sidekick[i].y,
                                                               *mouseX_,
                                                               *mouseY_,
                                                               this_option->wpnum + this_player->sidekick[i].charge,
                                                               playerNum_);
    ...
    if((playerNum_ == 2 || !twoPlayerMode) && !endLevel) {
        for(uint i = 0; i < COUNTOF(this_player->sidekick); ++i) {
            ...
            if(this_option->option > 0) {
                ...
                const int x = this_player->sidekick[i].x, y = this_player->sidekick[i].y;
                const uint sprite =
                    this_option->gr[this_player->sidekick[i].animation_frame] + this_player->sidekick[i].charge;
...
void JE_mainGamePlayerFunctions(void)
{
    ...
    if(twoPlayerMode) {
        JE_playerMovement(&player[0], !galagaMode ? inputDevice[0] : 0, 1, shipGr, shipGrPtr, &mouseX, &mouseY);
        JE_playerMovement(&player[1], !galagaMode ? inputDevice[1] : 0, 2, shipGr2, shipGr2ptr, &mouseXB, &mouseYB);
    } else {
        JE_playerMovement(&player[0], 0, 1, shipGr, shipGrPtr, &mouseX, &mouseY);
    }
...
void JE_playerCollide(Player *this_player, JE_byte playerNum_)
{
    ...
    for(int z = 0; z < 100; z++) {
        if(enemyAvail[z] != 1) {
            ...
            if(abs(this_player->x - enemy_screen_x) < 12 && abs(this_player->y - enemy[z].ey) < 14) { /*Collide*/
                ...
                if(evalue > 29999) {
                    if(evalue == 30000)  // spawn dragonwing in galaga mode, otherwise just a purple ball
                    {
                        ...
                        if(playerNum_ == 1) {
                            ...
                            if(isNetworkGame)
                                sprintf(
                                    tempStr, "%s %s %s", JE_getName(1), miscTextB[4 - 1], special[evalue - 32100].name);
                        ...
                        if(playerNum_ == 2) {
                            ...
                            if(isNetworkGame)
                                sprintf(
                                    tempStr, "%s %s %s", JE_getName(2), miscTextB[4 - 1], options[evalue - 32000].name);
                            else
                                sprintf(tempStr, "%s %s", miscText[44 - 1], options[evalue - 32000].name);
                            JE_drawTextWindow(tempStr);

                            // if picked up a different sidekick than player already has, then reset sidekicks to least
                            // powerful, else power them up
                    ...
                    } else if(evalue == -2)  // got rear weapon powerup
                    {
                        ...
                        shotMultiPos[SHOT_MISC] = 0;
                        b = player_shot_create(
                            0, SHOT_MISC, this_player->x, this_player->y, mouseX, mouseY, 104, playerNum_);
                        shotAvail[z] = 0;
                    } else if(evalue == -4) {
                    ...
                    JE_setupExplosion(
                        enemy_screen_x, enemy[z].ey, 0, enemyDat[enemy[z].enemytype].explosiontype, true, false);
                } else if(this_player->invulnerable_ticks == 0 && enemyAvail[z] == 0 &&
                          (enemyDat[enemy[z].enemytype].explosiontype & 1) ==
                              0)  // explosiontype & 1 == 0: not ground enemy
                {
                                ...
                                if(temp2 == b ||
                                   (temp != 255 && (temp == temp3 || temp - 100 == temp3 ||
                                    ...
                                    if(enemyDat[enemy[temp2].enemytype].esize == 1) {
                                        JE_setupExplosionLarge(enemy[temp2].enemyground,
                                                               enemy[temp2].explonum,
                                                               enemy_screen_x,
                                                               enemy[temp2].ey);
                                        soundQueue[6] = S_EXPLOSION_9;
                                    } else {
