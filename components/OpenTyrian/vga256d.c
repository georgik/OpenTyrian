...
{
    if(a < surface->pitch && b < surface->h && c < surface->pitch && d < surface->h) {
        Uint8 *vga = (Uint8 *) surface->pixels;
        int i;
...
void draw_segmented_gauge(SDL_Surface *surface,
                          int x,
                          int y,
                          Uint8 color,
                          uint segment_width,
                          ...
                          uint value)
{
    ...
    const uint segments = value / segment_value, partial_segment = value % segment_value;
