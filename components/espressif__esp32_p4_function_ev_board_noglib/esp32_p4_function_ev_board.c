...
// TX-only audio initialization to save memory (for games that don't need audio input)
esp_err_t bsp_audio_init_tx_only(const i2s_std_config_t *i2s_config)
{
    if (i2s_tx_chan) {
        /* Audio was initialized before */
        return ESP_OK;
    ...
    /* Setup I2S peripheral - TX channel only */
    i2s_chan_config_t chan_cfg = I2S_CHANNEL_DEFAULT_CONFIG(CONFIG_BSP_I2S_NUM, I2S_ROLE_MASTER);
    chan_cfg.auto_clear = true;
    ESP_ERROR_CHECK(i2s_new_channel(&chan_cfg, &i2s_tx_chan, NULL));  // NULL for RX

    /* Setup I2S TX channel - 22.05kHz mono to save DMA memory
     * OpenTyrian's 44.1kHz audio will be downsampled to 22.05kHz
     * Using mono mode (not stereo) to save 50% of DMA memory */
    i2s_std_config_t tx_cfg_default = {
        .clk_cfg = I2S_STD_CLK_DEFAULT_CONFIG(22050),  // 22.05kHz (saves DMA memory)
        .slot_cfg = I2S_STD_PHILIP_SLOT_DEFAULT_CONFIG(I2S_DATA_BIT_WIDTH_16BIT, I2S_SLOT_MODE_MONO),
        .gpio_cfg = BSP_I2S_GPIO_CFG,
    };

    const i2s_std_config_t *p_i2s_cfg = &tx_cfg_default;
    if (i2s_config != NULL) {
        p_i2s_cfg = i2s_config;
    }
...
esp_codec_dev_handle_t bsp_audio_codec_speaker_init_tx_only(void)
{
    if (i2s_data_if == NULL) {
        /* Initialize I2C */
        ESP_ERROR_CHECK(bsp_i2c_init());
        /* Configure I2S TX-only peripheral */
        ESP_ERROR_CHECK(bsp_audio_init_tx_only(NULL));
    }
...
#endif
            .dsi_bus = {
                .phy_clk_src = MIPI_DSI_PHY_PLLREF_CLK_SRC_DEFAULT, // Use default clock source (XTAL for ESP-IDF 6.1)
