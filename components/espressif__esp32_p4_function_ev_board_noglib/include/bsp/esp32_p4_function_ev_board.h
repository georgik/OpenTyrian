...
/**
 * @brief Initialize I2S audio with TX-only channel (saves memory)
 *
 * This function initializes only the I2S TX channel for audio output.
 * Use this for applications that don't need audio input to save DMA memory.
 *
 * @param[in] i2s_config I2S configuration (NULL for default 22.05kHz mono)
 * @return
 *      - ESP_OK               Initialize I2S successfully
 *      - ESP_ERR_INVALID_ARG  Initialize I2S failed because of invalid argument
 *      - ESP_ERR_NO_MEM       Initialize I2S failed because out of memory
 *      - ESP_ERR_INVALID_STATE This channel has not initialized or already started
 */
...
/**
 * @brief Initialize speaker codec device (TX-only to save memory)
 *
 * This function initializes only the I2S TX channel, not RX. Use this for applications
 * that don't need audio input (microphone) to save DMA memory.
 *
 * @return Pointer to codec device handle or NULL when error occurred
 */
